...
Project status: Phase 1 (Monitoring) and Phase 2 (Closed-Loop Control) both complete and running on hardware. Live closed-loop control verified with real water moving between two tanks. See the Roadmap below.
...
Fill pump P-101-Fill

Drain pump P-101-Drain
...
Triggers the ultrasonic sensor, takes a median of 5 readings, converts to distance.
Converts distance to a level using the calibration constants below, then applies an EMA filter to smooth noise.
Reads the operator setpoint and deadband from Modbus and runs bang-bang control on the two pumps.
Enforces a safety layer with hard-coded fault precedence (see below).
Serves everything as a Modbus TCP server on port 502.
Calibration
...
Sensor fail (consecutive missed echoes) -> both pumps OFF. Overflow logic cannot run on stale data.
Dry-run latch (pump running past the timeout) -> both pumps OFF, latched until the operator writes a new setpoint.
Overflow (level above OVERFLOW_CM, with hysteresis: clears below HIGH_ALARM_CM) -> drain FORCED ON regardless of setpoint.
Otherwise -> normal bang-bang control.
...
How to reproduce
Wire the HC-SR04 to the ESP32 as shown above (keep the Echo voltage divider). For Phase 2, add the relay and pumps, with each pump in series through its relay channel (COM/NO).
Open the firmware in the Arduino IDE (Core 3.x), install the modbus-esp8266 library, and set your WiFi credentials.
With SIMULATE_LEVEL = true, verify the control loop and safety logic in the Serial Monitor before connecting any hardware.
Mount the sensor in its final position, then measure and update EMPTY_DIST_CM and FULL_DIST_CM for your tank.
Upload, note the IP printed to serial, add a Modbus TCP device in Ignition pointing at that IP on port 502, configure the address mapping and tags above, and build the Perspective view.
Roadmap
